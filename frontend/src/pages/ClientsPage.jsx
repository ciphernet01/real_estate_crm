  ...
  return (
    <div style={{ display: 'grid', gap: '32px' }}>
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h2 style={{ fontSize: '1.75rem', fontWeight: 800, margin: 0, color: '#f8fafc' }}>Client Portfolio</h2>
          <p style={{ color: '#94a3b8', margin: '4px 0 0' }}>Manage buyer and seller relationships with unified activity logs</p>
        </div>
        <button 
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '24px' }}>
        {[
          ...
          <div key={s.label} className="premium-card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div style={{ width: '48px', height: '48px', borderRadius: '14px', background: 'rgba(255,255,255,0.03)', border: '1px solid var(--border-glass)', display: 'grid', placeItems: 'center', color: '#818cf8' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '20px' }}>
          ...
          {clients.map(client => (
            <article 
              ...
              style={{ cursor: 'pointer', transition: 'all 0.2s', border: activeClientId === client.id ? '2px solid #818cf8' : '1px solid var(--border-glass)', background: activeClientId === client.id ? 'rgba(129, 140, 248, 0.05)' : 'var(--bg-glass)' }}
            ...
            >
              <div style={{ display: 'flex', gap: '12px', alignItems: 'center', marginBottom: '16px' }}>
                ...
                <div style={{ flex: 1 }}>
                  <strong style={{ display: 'block', fontSize: '0.95rem', color: '#f8fafc' }}>{client.name}</strong>
                  <span style={{ fontSize: '0.7rem', fontWeight: 800, color: client.type === 'BUYER' ? '#818cf8' : '#f43f5e', background: client.type === 'BUYER' ? 'rgba(129, 140, 248, 0.1)' : 'rgba(244, 63, 94, 0.1)', padding: '2px 6px', borderRadius: '4px' }}>{client.type}</span>
                </div>
              </div>
              
              <div style={{ display: 'grid', gap: '10px', fontSize: '0.8rem', color: '#94a3b8', marginBottom: '16px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '16px', borderTop: '1px solid var(--border-glass)' }}>
                <span style={{ fontSize: '0.75rem', color: '#64748b' }}>{client.dealsCount || 0} active deals</span>
                <button 
                  onClick={(e) => { e.stopPropagation(); setClientForm(client); setIsFormOpen(true); }}
                  style={{ background: 'none', border: 'none', color: '#818cf8', fontSize: '0.75rem', fontWeight: 700, cursor: 'pointer' }}
                >
                  Edit profile
                </button>
              </div>
            ...
            <>
              <article className="premium-card" style={{ padding: '0' }}>
                <div style={{ padding: '24px', borderBottom: '1px solid var(--border-glass)' }}>
                  <h3 style={{ margin: '0 0 16px', fontSize: '1rem', fontWeight: 800, color: '#f8fafc' }}>Activity Log</h3>
                  <div style={{ display: 'flex', gap: '8px' }}>
                    ...
                    <button 
                      ...
                      style={{ height: '42px', padding: '0 12px', background: '#818cf8', color: 'white', borderRadius: '8px', border: 'none', cursor: 'pointer' }}
                    >
                ...
                <div style={{ padding: '24px', display: 'grid', gap: '20px', maxHeight: '400px', overflowY: 'auto' }}>
                  ...
                  {details?.interactions?.map(item => (
                    <div key={item.id} style={{ display: 'flex', gap: '12px' }}>
                      <div style={{ width: '4px', height: 'auto', background: 'var(--border-glass)', borderRadius: '2px' }}></div>
                      <div>
                        <p style={{ margin: '0 0 4px', fontSize: '0.85rem', color: '#f8fafc' }}>{item.content}</p>
              ...
              <article className="premium-card">
                <h3 style={{ margin: '0 0 16px', fontSize: '1rem', fontWeight: 800, color: '#f8fafc' }}>Linked Assets</h3>
                <div style={{ display: 'grid', gap: '12px' }}>
                  {details?.leads?.map(lead => (
                    <div key={lead.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px', background: 'rgba(255,255,255,0.03)', border: '1px solid var(--border-glass)', borderRadius: '10px' }}>
                      <div>
                        <strong style={{ display: 'block', fontSize: '0.8rem', color: '#f8fafc' }}>{lead.name}</strong>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0, 0, 0, 0.7)', backdropFilter: 'blur(12px)', display: 'grid', placeItems: 'center', zIndex: 1000 }}>
          <div className="premium-card" style={{ width: 'min(500px, 95%)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '24px' }}>
              <h3 style={{ margin: 0, fontSize: '1.25rem', fontWeight: 800, color: '#f8fafc' }}>{clientForm.id ? 'Edit Client' : 'New Client'}</h3>
              <button 
            ...
            <form onSubmit={e => { e.preventDefault(); saveClientMutation.mutate(clientForm); }} style={{ display: 'grid', gap: '20px' }}>
              <label>
                <span style={{ fontSize: '0.75rem', fontWeight: 700, color: '#94a3b8' }}>FULL NAME</span>
                <input value={clientForm.name} onChange={e => setClientForm(f => ({ ...f, name: e.target.value }))} required />
              </label>
              <label>
                <span style={{ fontSize: '0.75rem', fontWeight: 700, color: '#94a3b8' }}>ACCOUNT TYPE</span>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                ...
                <label>
                  <span style={{ fontSize: '0.75rem', fontWeight: 700, color: '#94a3b8' }}>PHONE</span>
                  <input value={clientForm.phone} onChange={e => setClientForm(f => ({ ...f, phone: e.target.value }))} />
                </label>
              </div>
              <label>
                <span style={{ fontSize: '0.75rem', fontWeight: 700, color: '#94a3b8' }}>PREFERENCES / NOTES</span>
                <textarea 
